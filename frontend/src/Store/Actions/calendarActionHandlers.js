  ...
  [types.FETCH_CALENDAR]: function(payload) {
    return function(dispatch, getState) {
      ...
      if (view === calendarViews.DAY) {
        start = moment(start).subtract(1, 'day').toISOString();
        end = moment(end).add(1, 'day').toISOString();
      } else if (view === calendarViews.WEEK || view === calendarViews.FORECAST) {
        start = moment(start).subtract(1, 'week').toISOString();
        end = moment(end).add(1, 'week').toISOString();
      }
